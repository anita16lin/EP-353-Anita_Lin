...
void schedulePattern(Note* originalPattern, Note* newPattern, int originalPatternLength, float startBeat, int numberOfRepetitions, int repetitionInterval) {
	printf("Hello!\n");
...
int main() {
	...
	Note newPattern[32];
	int newPatternLength = sizeof(newPattern)/sizeof(newPattern[0]);
	schedulePattern(pattern01, newPattern, 12, 2., 2, 8);
	for (int i = 0; i < 16; ++i)
	{
		printf("newPatternNote: %f\n", newPattern[i].beat);
	}
	...
	for (int tick = 0; tick < totalBeats * ticksPerBeat; ++tick)
	{
		...
		if (beat >= newPattern[pattern01Index].beat && pattern01Index < newPatternLength)
		{
			Note currentNote = newPattern[pattern01Index];
